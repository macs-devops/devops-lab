...
resource "aws_kms_key" "this" {
  ...
  tags = {
    Name        = "${var.aws_root_name}-kms-teraform-${var.region}"
...
resource "aws_kms_alias" "this" {
  name          = "alias/${var.aws_root_name}-kms-terraform-${var.region}"
  target_key_id = aws_kms_key.this.id
}

# Provides an S3 bucket for storing Terraform state files
resource "aws_s3_bucket" "this" {
  bucket = "${var.aws_root_name}-s3-terraform-${var.region}"
  acl    = "private"
  versioning {
    enabled = true
  }
  ...
  tags = {
    Name        = "${var.aws_root_name}-s3-terraform-${var.region}"
    Environment = var.aws_environment_name
    Source      = var.aws_source_name
    Answer      = "42"
    #tfsec:ignore:AWS002
